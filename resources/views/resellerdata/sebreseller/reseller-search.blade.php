...
@section('reseller-content')
    ...
    <a href="{{route('reseller.create.subreseller')}}" class="btn btn-primary pull-left">
        <i class="entypo-plus"></i>
        Create New
    </a>
    <div id="table-1_filter" class="dataTables_filter pull-right">
        <form action="{{route('reseller.subreseller.search')}}" method="post">
            @csrf
            <label>Search:
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <table class="table table-bordered responsive">
                ...
                <tbody>
                @if(count($user) > 0)
                    @foreach($user as $subresl)
                        <tr>
                            <td>{{$subresl->name}}</td>
                            <td>{{$subresl->user_name}}</td>
                            <td>{{ $subresl->pass_rep}}</td>
                            @if(!empty($subresl->reseller_id))
                            ...
                            @endif
                            <td>
                                <a href="{{route('reseller.sub.reseller.edit',$subresl->id)}}" class="btn btn-default btn-sm btn-icon icon-left">
                        ...
                        <div class="modal fade custom-width modalfate" id="subreseller-delete{{$subresl->id}}">
                            <div class="modal-dialog" style="width: 60%;">
                                <form action="{{route('reseller.subreseller.delete')}}" method="post">
                                    @csrf
                        ...
                        <div class="modal fade custom-width modalfate" id="sub-subreseller-block{{$subresl->id}}">
                            <div class="modal-dialog" style="width: 60%;">
                                <form action="{{route('reseller.subreseller.block')}}" method="post">
                                    @csrf
                                    <div class="modal-content">
                        ...
                        <div class="modal fade custom-width modalfate" id="sub-subreseller-unblock{{$subresl->id}}">
                            <div class="modal-dialog" style="width: 60%;">
                                <form action="{{route('reseller.subreseller.unblock')}}" method="post">
                                    @csrf
                                    <div class="modal-content">
                        ...
                        <div class="modal fade custom-width modalfate" id="sub-reseller-add-cradit{{$subresl->id}}">
                            <div class="modal-dialog" style="width: 60%;">
                                <form action="{{route('reseller.subreseller.add.credit.bal')}}" method="post">
                                    @csrf
                                    <div class="modal-content">
