...
@section('administrator-content')
    ...
    <div id="table-1_filter" class="dataTables_filter pull-right">
        <form action="{{route('adminintrator.subreseller.search')}}" method="post">
            @csrfadminintrator.subreseller.search
            <label>Search:
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <table class="table table-bordered responsive">
                ...
                <tbody>
                @if(count($sureseller_search) > 0)
                    @foreach($sureseller_search as $subresl)
                        <tr>
                            <td>{{$subresl->name}}</td>
                            <td>{{$subresl->user_name}}</td>
                            <td>{{ $subresl->pass_rep}}</td>
                            @if(!empty($subresl->administrator_id))
                            ...
                            <td>
                                ...
                                <a href="{{route('adminstrator.subreseller.chnageper',$subresl->id)}}" class="btn btn-info btn-sm btn-icon icon-left">
                        ...
                        <div class="modal fade custom-width modalfate" id="subreseller-delete{{$subresl->id}}">
                            <div class="modal-dialog" style="width: 60%;">
                                <form action="{{route('adminstrator.subreseller.delete')}}" method="post">
                                    @csrf
                        ...
                        <div class="modal fade custom-width modalfate" id="sub-subreseller-block{{$subresl->id}}">
                            <div class="modal-dialog" style="width: 60%;">
                                <form action="{{route('adminstrator.subreseller.block')}}" method="post">
                                    @csrf
                                    <div class="modal-content">
                        ...
                        <div class="modal fade custom-width modalfate" id="sub-subreseller-unblock{{$subresl->id}}">
                            <div class="modal-dialog" style="width: 60%;">
                                <form action="{{route('adminstrator.subreseller.unblock')}}" method="post">
                                    @csrf
                                    <div class="modal-content">
                        ...
                        <div class="modal fade custom-width modalfate" id="sub-reseller-add-cradit{{$subresl->id}}">
                            <div class="modal-dialog" style="width: 60%;">
                                <form action="{{route('adminstrator.subreseller.add.credit.bal')}}" method="post">
                                    @csrf
                                    <div class="modal-content">
